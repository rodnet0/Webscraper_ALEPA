...
driver.get("https://alepa.pa.gov.br/Transparencia/Page/VerbaIndenizatoria")
sleep(3)

#essa funcao formata o valor da diaria, para que o valor possa ser inserido no banco de dados
def converter_valor_verba_indenizatoria(valor_verba_indenizatoria):
    valor_verba_indenizatoria = str(valor_verba_indenizatoria)   
    valor_verba_indenizatoria = valor_verba_indenizatoria.replace("R$ ", "")
    valor_verba_indenizatoria = valor_verba_indenizatoria.replace(".", "")
    valor_verba_indenizatoria = valor_verba_indenizatoria.replace(",", ".")
    valor_verba_indenizatoria = float(valor_verba_indenizatoria)  
    return valor_verba_indenizatoria
...
def inserir_verba_indenizatoria(celulas_tabela):
    lista_celulas = []
    ...
    lista_celulas[-1] = converter_valor_verba_indenizatoria(lista_celulas[-1])
    ...
    sql = """
        INSERT INTO verba_indenizatoria (mes_referencia, nome_deputado, classificacao_verba, descricao_verba, valor_verba) VALUES (%s, %s, %s, %s, %s)
"""
...
for i in range(3):
    ...
    for j in range(1, numero_linhas + 1):
        ...
        inserir_verba_indenizatoria(celulas_tabela)
        for celula in celulas_tabela:
