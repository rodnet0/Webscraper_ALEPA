...
df = conexao.query('SELECT nome_deputado, valor_verba, mes_referencia FROM verba_gabinete;', ttl="10m")

df.columns = ["Deputado", "Valor", "Data"]
...
st.write("### Verba de Gabinete Entre os Anos de 2021 a 2024")
...
df = df.query("Deputado == @deputado and (Data >= @data_inicio and Data <= @data_fim)").sort_values(["Deputado", "Data"])
df_exibicao = df.copy()
